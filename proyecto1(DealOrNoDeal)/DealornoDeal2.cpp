        ...
        switch(opcion){
            case 1://jugar
               ...
               gets(nombre);
                ...
                dinero=jugar();
                printf("Felicidades ganaste %d",dinero);
...
int jugar(){
    ...
    int num_aleatorio;
    int numeros [TAM]={-1};
    int fstop, maletin,umaletin;
    int i,j,e,banca1,flag;
    int suma=0;
    ...
    for(j=0;j<9;j++){
       ...
       e=A[j];
        ...
        printf("Maletin  ");
        scanf("%d",&mal);
        if (numeros[mal-1]==0||mal==malel||mal<1||mal>26){
            printf("maletin incorrecto\n\n");
            i--;
        }
        else {
        printf("\tEl maletin tenia %d\n\n",numeros[mal-1]);
        numeros[mal-1]=0;
        C[mal-1]=0;
        e--;
        }
        }
        banca1=banca(numeros);
        printf("La oferta de la banca es: $%d\n",banca1);
        printf("\t1.-Deal\t\t2.-No Deal\nOpcion: ");
        scanf("%i",&flag);
        if (flag==1){
			return banca1;
			}
   ...
   do{
    printf("1.-Te quedas con tu maletin %d\n2.-Cambias al maletin %d \nOpcion:",malel,umalel);
    scanf("%i",&flag);
    switch(flag){
    case 1:
        printf("\n\t\tEl maletin %d tenia $%d\n",umalel,umaletin);
		return maletin;
		break;
		case 2:
        printf("\n\t\tEl maletin %d tenia $%d\n",malel,maletin);
